...
func TestDNSSECInstantTestsAPIService_CreateDnsSecInstantTest(t *testing.T) {
	...
	var requestBody DnsSecInstantTestRequest
	...
	var expectedResponse DnsSecInstantTestResponse
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		expectedPath := "/tests/dnssec/instant"
		...
		var receivedBody DnsSecInstantTestRequest
	...
	api := (*DNSSECInstantTestsAPIService)(&apiClient.Common)

	actualResponse, httpResponse, err := api.CreateDnsSecInstantTest().DnsSecInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
	if err != nil {
		t.Fatalf("CreateDnsSecInstantTest() error = %v", err)
	}
	if httpResponse == nil {
		t.Fatal("CreateDnsSecInstantTest() returned a nil HTTP response")
	}
	if httpResponse.StatusCode != responseStatus {
		t.Errorf("CreateDnsSecInstantTest() status = %d, want %d", httpResponse.StatusCode, responseStatus)
	}
	expected := &expectedResponse
	if !reflect.DeepEqual(actualResponse, expected) {
		t.Errorf("CreateDnsSecInstantTest() response = %#v, want %#v", actualResponse, expected)
	}
}

func TestDNSSECInstantTestsAPIService_CreateDnsSecInstantTest_Errors(t *testing.T) {
	const testToken = "test-token"
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.CreateDnsSecInstantTest().DnsSecInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
		if err == nil {
			t.Fatalf("CreateDnsSecInstantTest() error = nil, want *GenericAPIError")
		}
		if httpResponse == nil {
			t.Fatal("CreateDnsSecInstantTest() returned a nil HTTP response")
		}
		...
		if !errors.As(err, &apiError) {
			t.Fatalf("CreateDnsSecInstantTest() error type = %T, want *GenericAPIError", err)
		}
