...
func TestHTTPServerInstantTestsAPIService_CreateHttpServerInstantTest(t *testing.T) {
	...
	var requestBody HttpServerInstantTestRequest
	...
	var expectedResponse HttpServerInstantTestResponse
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		expectedPath := "/tests/http-server/instant"
		...
		var receivedBody HttpServerInstantTestRequest
	...
	api := (*HTTPServerInstantTestsAPIService)(&apiClient.Common)

	actualResponse, httpResponse, err := api.CreateHttpServerInstantTest().HttpServerInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
	if err != nil {
		t.Fatalf("CreateHttpServerInstantTest() error = %v", err)
	}
	if httpResponse == nil {
		t.Fatal("CreateHttpServerInstantTest() returned a nil HTTP response")
	}
	if httpResponse.StatusCode != responseStatus {
		t.Errorf("CreateHttpServerInstantTest() status = %d, want %d", httpResponse.StatusCode, responseStatus)
	}
	expected := &expectedResponse
	if !reflect.DeepEqual(actualResponse, expected) {
		t.Errorf("CreateHttpServerInstantTest() response = %#v, want %#v", actualResponse, expected)
	}
}

func TestHTTPServerInstantTestsAPIService_CreateHttpServerInstantTest_Errors(t *testing.T) {
	const testToken = "test-token"
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.CreateHttpServerInstantTest().HttpServerInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
		if err == nil {
			t.Fatalf("CreateHttpServerInstantTest() error = nil, want *GenericAPIError")
		}
		if httpResponse == nil {
			t.Fatal("CreateHttpServerInstantTest() returned a nil HTTP response")
		}
		...
		if !errors.As(err, &apiError) {
			t.Fatalf("CreateHttpServerInstantTest() error type = %T, want *GenericAPIError", err)
		}
