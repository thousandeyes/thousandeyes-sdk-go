...
func TestWebTransactionInstantTestsAPIService_CreateWebTransactionInstantTest(t *testing.T) {
	...
	var requestBody WebTransactionInstantTestRequest
	...
	var expectedResponse WebTransactionInstantTestResponse
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		expectedPath := "/tests/web-transactions/instant"
		...
		var receivedBody WebTransactionInstantTestRequest
	...
	api := (*WebTransactionInstantTestsAPIService)(&apiClient.Common)

	actualResponse, httpResponse, err := api.CreateWebTransactionInstantTest().WebTransactionInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
	if err != nil {
		t.Fatalf("CreateWebTransactionInstantTest() error = %v", err)
	}
	if httpResponse == nil {
		t.Fatal("CreateWebTransactionInstantTest() returned a nil HTTP response")
	}
	if httpResponse.StatusCode != responseStatus {
		t.Errorf("CreateWebTransactionInstantTest() status = %d, want %d", httpResponse.StatusCode, responseStatus)
	}
	expected := &expectedResponse
	if !reflect.DeepEqual(actualResponse, expected) {
		t.Errorf("CreateWebTransactionInstantTest() response = %#v, want %#v", actualResponse, expected)
	}
}

func TestWebTransactionInstantTestsAPIService_CreateWebTransactionInstantTest_Errors(t *testing.T) {
	const testToken = "test-token"
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.CreateWebTransactionInstantTest().WebTransactionInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
		if err == nil {
			t.Fatalf("CreateWebTransactionInstantTest() error = nil, want *GenericAPIError")
		}
		if httpResponse == nil {
			t.Fatal("CreateWebTransactionInstantTest() returned a nil HTTP response")
		}
		...
		if !errors.As(err, &apiError) {
			t.Fatalf("CreateWebTransactionInstantTest() error type = %T, want *GenericAPIError", err)
		}
