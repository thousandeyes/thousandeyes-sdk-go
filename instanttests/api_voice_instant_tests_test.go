...
func TestVoiceInstantTestsAPIService_CreateVoiceInstantTest(t *testing.T) {
	...
	var requestBody VoiceInstantTestRequest
	...
	var expectedResponse VoiceInstantTestResponse
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		expectedPath := "/tests/voice/instant"
		...
		var receivedBody VoiceInstantTestRequest
	...
	api := (*VoiceInstantTestsAPIService)(&apiClient.Common)

	actualResponse, httpResponse, err := api.CreateVoiceInstantTest().VoiceInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
	if err != nil {
		t.Fatalf("CreateVoiceInstantTest() error = %v", err)
	}
	if httpResponse == nil {
		t.Fatal("CreateVoiceInstantTest() returned a nil HTTP response")
	}
	if httpResponse.StatusCode != responseStatus {
		t.Errorf("CreateVoiceInstantTest() status = %d, want %d", httpResponse.StatusCode, responseStatus)
	}
	expected := &expectedResponse
	if !reflect.DeepEqual(actualResponse, expected) {
		t.Errorf("CreateVoiceInstantTest() response = %#v, want %#v", actualResponse, expected)
	}
}

func TestVoiceInstantTestsAPIService_CreateVoiceInstantTest_Errors(t *testing.T) {
	const testToken = "test-token"
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.CreateVoiceInstantTest().VoiceInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
		if err == nil {
			t.Fatalf("CreateVoiceInstantTest() error = nil, want *GenericAPIError")
		}
		if httpResponse == nil {
			t.Fatal("CreateVoiceInstantTest() returned a nil HTTP response")
		}
		...
		if !errors.As(err, &apiError) {
			t.Fatalf("CreateVoiceInstantTest() error type = %T, want *GenericAPIError", err)
		}
