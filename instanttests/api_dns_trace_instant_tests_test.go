...
func TestDNSTraceInstantTestsAPIService_CreateDnsTraceInstantTest(t *testing.T) {
	...
	var requestBody DnsTraceInstantTestRequest
	...
	var expectedResponse DnsTraceInstantTestResponse
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		expectedPath := "/tests/dns-trace/instant"
		...
		var receivedBody DnsTraceInstantTestRequest
	...
	api := (*DNSTraceInstantTestsAPIService)(&apiClient.Common)

	actualResponse, httpResponse, err := api.CreateDnsTraceInstantTest().DnsTraceInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
	if err != nil {
		t.Fatalf("CreateDnsTraceInstantTest() error = %v", err)
	}
	if httpResponse == nil {
		t.Fatal("CreateDnsTraceInstantTest() returned a nil HTTP response")
	}
	if httpResponse.StatusCode != responseStatus {
		t.Errorf("CreateDnsTraceInstantTest() status = %d, want %d", httpResponse.StatusCode, responseStatus)
	}
	expected := &expectedResponse
	if !reflect.DeepEqual(actualResponse, expected) {
		t.Errorf("CreateDnsTraceInstantTest() response = %#v, want %#v", actualResponse, expected)
	}
}

func TestDNSTraceInstantTestsAPIService_CreateDnsTraceInstantTest_Errors(t *testing.T) {
	const testToken = "test-token"
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.CreateDnsTraceInstantTest().DnsTraceInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
		if err == nil {
			t.Fatalf("CreateDnsTraceInstantTest() error = nil, want *GenericAPIError")
		}
		if httpResponse == nil {
			t.Fatal("CreateDnsTraceInstantTest() returned a nil HTTP response")
		}
		...
		if !errors.As(err, &apiError) {
			t.Fatalf("CreateDnsTraceInstantTest() error type = %T, want *GenericAPIError", err)
		}
