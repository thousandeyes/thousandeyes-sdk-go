...
func TestSIPServerInstantTestsAPIService_CreateSipServerInstantTest(t *testing.T) {
	...
	var requestBody SipServerInstantTestRequest
	...
	var expectedResponse SipServerInstantTestResponse
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		expectedPath := "/tests/sip-server/instant"
		...
		var receivedBody SipServerInstantTestRequest
	...
	api := (*SIPServerInstantTestsAPIService)(&apiClient.Common)

	actualResponse, httpResponse, err := api.CreateSipServerInstantTest().SipServerInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
	if err != nil {
		t.Fatalf("CreateSipServerInstantTest() error = %v", err)
	}
	if httpResponse == nil {
		t.Fatal("CreateSipServerInstantTest() returned a nil HTTP response")
	}
	if httpResponse.StatusCode != responseStatus {
		t.Errorf("CreateSipServerInstantTest() status = %d, want %d", httpResponse.StatusCode, responseStatus)
	}
	expected := &expectedResponse
	if !reflect.DeepEqual(actualResponse, expected) {
		t.Errorf("CreateSipServerInstantTest() response = %#v, want %#v", actualResponse, expected)
	}
}

func TestSIPServerInstantTestsAPIService_CreateSipServerInstantTest_Errors(t *testing.T) {
	const testToken = "test-token"
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.CreateSipServerInstantTest().SipServerInstantTestRequest(requestBody).Aid(aid).Expand(expand).Execute()
		if err == nil {
			t.Fatalf("CreateSipServerInstantTest() error = nil, want *GenericAPIError")
		}
		if httpResponse == nil {
			t.Fatal("CreateSipServerInstantTest() returned a nil HTTP response")
		}
		...
		if !errors.As(err, &apiError) {
			t.Fatalf("CreateSipServerInstantTest() error type = %T, want *GenericAPIError", err)
		}
