...
// ApiListWidgetAsApiWidget is a convenience function that returns ApiListWidget wrapped in ApiWidget
func ApiListWidgetAsApiWidget(v *ApiListWidget) ApiWidget {
	return ApiWidget{
		ApiListWidget: v,
	}
}
...
func (dst *ApiWidget) UnmarshalJSON(data []byte) error {
	...
	if jsonDict["type"] == "List" {
		...
		err = json.Unmarshal(data, &dst.ApiListWidget)
		if err == nil {
			return nil // data stored in dst.ApiListWidget, return on the first match
		} else {
			dst.ApiListWidget = nil
			return fmt.Errorf("failed to unmarshal ApiWidget as ApiListWidget: %s", err.Error())
		}
	}
