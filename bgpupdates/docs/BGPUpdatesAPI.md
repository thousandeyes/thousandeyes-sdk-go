...
> BgpUpdates GetBgpUpdates().Aid(aid).Max(max).Cursor(cursor).Expand(expand).StartDate(startDate).EndDate(endDate).Prefixes(prefixes).OriginAses(originAses).AsPaths(asPaths).RpkiStatuses(rpkiStatuses).UpdateType(updateType).MonitorIds(monitorIds).Communities(communities).Execute()
...
func main() {
	...
	prefixes := []string{"Inner_example"} // []string | Prefix CIDR filters. Repeat the parameter to filter by multiple prefixes. (optional)
	originAses := []int64{int64(123)} // []int64 | Origin AS filters. Repeat the parameter to filter by multiple ASNs. (optional)
	asPaths := []string{"Inner_example"} // []string | AS path filters, expressed as a space-separated list of ASNs. Repeat the parameter to filter by multiple AS paths. (optional)
	rpkiStatuses := []bgpupdates.BgpRpkiStatus{bgpupdates.BgpRpkiStatus("Valid")} // []BgpRpkiStatus | RPKI status filters. (optional)
	updateType := []bgpupdates.BgpUpdateType{bgpupdates.BgpUpdateType("announcement")} // []BgpUpdateType | BGP update type filters. (optional)
	monitorIds := []string{"Inner_example"} // []string | BGP monitor ID filters. Repeat the parameter to filter by multiple monitors. Get `monitorId` from the `/monitors` endpoint. (optional)
	...
	resp, r, err := api.GetBgpUpdates().Aid(aid).Max(max).Cursor(cursor).Expand(expand).StartDate(startDate).EndDate(endDate).Prefixes(prefixes).OriginAses(originAses).AsPaths(asPaths).RpkiStatuses(rpkiStatuses).UpdateType(updateType).MonitorIds(monitorIds).Communities(communities).Execute()
	if err != nil {
 ...
 **prefixes** | **[]string** | Prefix CIDR filters. Repeat the parameter to filter by multiple prefixes. | 
 **originAses** | **[]int64** | Origin AS filters. Repeat the parameter to filter by multiple ASNs. | 
 **asPaths** | **[]string** | AS path filters, expressed as a space-separated list of ASNs. Repeat the parameter to filter by multiple AS paths. | 
 **rpkiStatuses** | [**[]BgpRpkiStatus**](BgpRpkiStatus.md) | RPKI status filters. | 
 **updateType** | [**[]BgpUpdateType**](BgpUpdateType.md) | BGP update type filters. | 
 **monitorIds** | **[]string** | BGP monitor ID filters. Repeat the parameter to filter by multiple monitors. Get &#x60;monitorId&#x60; from the &#x60;/monitors&#x60; endpoint. | 
