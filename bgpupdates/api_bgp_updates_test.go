...
func TestBGPUpdatesAPIService_GetBgpUpdates(t *testing.T) {
	...
	var prefixes []string
	var originAses []int64
	var asPaths []string
	var rpkiStatuses []BgpRpkiStatus
	...
	actualResponse, httpResponse, err := api.GetBgpUpdates().Aid(aid).Max(max).Cursor(cursor).Expand(expand).StartDate(startDate).EndDate(endDate).Prefixes(prefixes).OriginAses(originAses).AsPaths(asPaths).RpkiStatuses(rpkiStatuses).UpdateType(updateType).MonitorIds(monitorIds).Communities(communities).Execute()
	if err != nil {
...
func TestBGPUpdatesAPIService_GetBgpUpdates_Errors(t *testing.T) {
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.GetBgpUpdates().Aid(aid).Max(max).Cursor(cursor).Expand(expand).StartDate(startDate).EndDate(endDate).Prefixes(prefixes).OriginAses(originAses).AsPaths(asPaths).RpkiStatuses(rpkiStatuses).UpdateType(updateType).MonitorIds(monitorIds).Communities(communities).Execute()
		if err == nil {
