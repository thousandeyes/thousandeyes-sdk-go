...
> AgentToAgentTestResponse GetAgentToAgentTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
...
func main() {
	...
	resp, r, err := api.GetAgentToAgentTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
	if err != nil {
