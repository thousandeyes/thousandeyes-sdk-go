...
func main() {
	voiceTestRequest := *tests.NewVoiceTestRequest(tests.TestInterval(60), "281474976710706", []tests.TestAgentWithSourceIpRequest{*tests.NewTestAgentWithSourceIpRequest("AgentId_example")}) // VoiceTestRequest | 
...
> VoiceTestResponse GetVoiceTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
...
func main() {
	...
	resp, r, err := api.GetVoiceTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
	if err != nil {
