...
> WebTransactionTestResponse GetWebTransactionsTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
...
func main() {
	...
	resp, r, err := api.GetWebTransactionsTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
	if err != nil {
