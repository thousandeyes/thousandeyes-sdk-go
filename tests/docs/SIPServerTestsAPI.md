...
func main() {
	sipServerTestRequest := *tests.NewSipServerTestRequest(tests.TestInterval(60), []tests.TestAgentWithSourceIpRequest{*tests.NewTestAgentWithSourceIpRequest("AgentId_example")}, *tests.NewTestSipCredentials(int32(49153))) // SipServerTestRequest | 
...
> SipServerTestResponse GetSipServerTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
...
func main() {
	...
	resp, r, err := api.GetSipServerTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
	if err != nil {
