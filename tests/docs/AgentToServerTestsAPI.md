...
func main() {
	agentToServerTestRequest := *tests.NewAgentToServerTestRequest(tests.TestInterval(60), "www.thousandeyes.com:80", []tests.TestAgentWithSourceIpRequest{*tests.NewTestAgentWithSourceIpRequest("AgentId_example")}) // AgentToServerTestRequest | 
...
> AgentToServerTestResponse GetAgentToServerTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
...
func main() {
	...
	resp, r, err := api.GetAgentToServerTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
	if err != nil {
