...
func main() {
	ftpServerTestRequest := *tests.NewFtpServerTestRequest(tests.TestInterval(60), "password", tests.FtpServerRequestType("download"), "www.thousandeyes.com", "username", []tests.TestAgentWithSourceIpRequest{*tests.NewTestAgentWithSourceIpRequest("AgentId_example")}) // FtpServerTestRequest | 
...
> FtpServerTestResponse GetFtpServerTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
...
func main() {
	...
	resp, r, err := api.GetFtpServerTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
	if err != nil {
