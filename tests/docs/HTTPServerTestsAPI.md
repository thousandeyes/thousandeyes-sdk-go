...
func main() {
	httpServerTestRequest := *tests.NewHttpServerTestRequest(tests.TestInterval(60), "www.thousandeyes.com", []tests.TestAgentWithSourceIpRequest{*tests.NewTestAgentWithSourceIpRequest("AgentId_example")}) // HttpServerTestRequest | 
...
> HttpServerTestResponse GetHttpServerTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
...
func main() {
	...
	resp, r, err := api.GetHttpServerTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
	if err != nil {
