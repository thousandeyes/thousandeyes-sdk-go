...
> PageLoadTestResponse GetPageLoadTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
...
func main() {
	...
	resp, r, err := api.GetPageLoadTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
	if err != nil {
