...
> DnsTraceTestResponse GetDnsTraceTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
...
func main() {
	...
	resp, r, err := api.GetDnsTraceTest(testId).Aid(aid).VersionId(versionId).Expand(expand).Execute()
	if err != nil {
