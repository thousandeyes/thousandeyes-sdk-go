/*
Endpoint Test Results API

Retrieve results for scheduled and dynamic tests on endpoint agents.

API version: 7.0.68
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package endpointtestresults

import (
	"encoding/json"
    "github.com/thousandeyes/thousandeyes-sdk-go/v3/internal/utils"
)

// checks if the VpnNetworkPing type satisfies the MappedNullable interface at compile time
var _ utils.MappedNullable = &VpnNetworkPing{}

// VpnNetworkPing struct for VpnNetworkPing
type VpnNetworkPing struct {
	// Ping average response time.
	AvgRtt *int32 `json:"avgRtt,omitempty"`
	// Ping maximum response time.
	MaxRtt *int32 `json:"maxRtt,omitempty"`
	// Network loss.
	Loss *float64 `json:"loss,omitempty"`
	// Ping mean standard deviation response time.
	MeanDevRtt *int32 `json:"meanDevRtt,omitempty"`
	// Ping minimum response time.
	MinRtt *int32 `json:"minRtt,omitempty"`
	// Ping packets received.
	PktsReceived *int32 `json:"pktsReceived,omitempty"`
	// Ping packets sent.
	PktsSent *int32 `json:"pktsSent,omitempty"`
	// Only present when there is an error.
	Error *string `json:"error,omitempty"`
	InfoFlags []string `json:"infoFlags,omitempty"`
}

// NewVpnNetworkPing instantiates a new VpnNetworkPing object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewVpnNetworkPing() *VpnNetworkPing {
	this := VpnNetworkPing{}
	return &this
}

// NewVpnNetworkPingWithDefaults instantiates a new VpnNetworkPing object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewVpnNetworkPingWithDefaults() *VpnNetworkPing {
	this := VpnNetworkPing{}
	return &this
}

// GetAvgRtt returns the AvgRtt field value if set, zero value otherwise.
func (o *VpnNetworkPing) GetAvgRtt() int32 {
	if o == nil || utils.IsNil(o.AvgRtt) {
		var ret int32
		return ret
	}
	return *o.AvgRtt
}

// GetAvgRttOk returns a tuple with the AvgRtt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VpnNetworkPing) GetAvgRttOk() (*int32, bool) {
	if o == nil || utils.IsNil(o.AvgRtt) {
		return nil, false
	}
	return o.AvgRtt, true
}

// HasAvgRtt returns a boolean if a field has been set.
func (o *VpnNetworkPing) HasAvgRtt() bool {
	if o != nil && !utils.IsNil(o.AvgRtt) {
		return true
	}

	return false
}

// SetAvgRtt gets a reference to the given int32 and assigns it to the AvgRtt field.
func (o *VpnNetworkPing) SetAvgRtt(v int32) {
	o.AvgRtt = &v
}

// GetMaxRtt returns the MaxRtt field value if set, zero value otherwise.
func (o *VpnNetworkPing) GetMaxRtt() int32 {
	if o == nil || utils.IsNil(o.MaxRtt) {
		var ret int32
		return ret
	}
	return *o.MaxRtt
}

// GetMaxRttOk returns a tuple with the MaxRtt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VpnNetworkPing) GetMaxRttOk() (*int32, bool) {
	if o == nil || utils.IsNil(o.MaxRtt) {
		return nil, false
	}
	return o.MaxRtt, true
}

// HasMaxRtt returns a boolean if a field has been set.
func (o *VpnNetworkPing) HasMaxRtt() bool {
	if o != nil && !utils.IsNil(o.MaxRtt) {
		return true
	}

	return false
}

// SetMaxRtt gets a reference to the given int32 and assigns it to the MaxRtt field.
func (o *VpnNetworkPing) SetMaxRtt(v int32) {
	o.MaxRtt = &v
}

// GetLoss returns the Loss field value if set, zero value otherwise.
func (o *VpnNetworkPing) GetLoss() float64 {
	if o == nil || utils.IsNil(o.Loss) {
		var ret float64
		return ret
	}
	return *o.Loss
}

// GetLossOk returns a tuple with the Loss field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VpnNetworkPing) GetLossOk() (*float64, bool) {
	if o == nil || utils.IsNil(o.Loss) {
		return nil, false
	}
	return o.Loss, true
}

// HasLoss returns a boolean if a field has been set.
func (o *VpnNetworkPing) HasLoss() bool {
	if o != nil && !utils.IsNil(o.Loss) {
		return true
	}

	return false
}

// SetLoss gets a reference to the given float64 and assigns it to the Loss field.
func (o *VpnNetworkPing) SetLoss(v float64) {
	o.Loss = &v
}

// GetMeanDevRtt returns the MeanDevRtt field value if set, zero value otherwise.
func (o *VpnNetworkPing) GetMeanDevRtt() int32 {
	if o == nil || utils.IsNil(o.MeanDevRtt) {
		var ret int32
		return ret
	}
	return *o.MeanDevRtt
}

// GetMeanDevRttOk returns a tuple with the MeanDevRtt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VpnNetworkPing) GetMeanDevRttOk() (*int32, bool) {
	if o == nil || utils.IsNil(o.MeanDevRtt) {
		return nil, false
	}
	return o.MeanDevRtt, true
}

// HasMeanDevRtt returns a boolean if a field has been set.
func (o *VpnNetworkPing) HasMeanDevRtt() bool {
	if o != nil && !utils.IsNil(o.MeanDevRtt) {
		return true
	}

	return false
}

// SetMeanDevRtt gets a reference to the given int32 and assigns it to the MeanDevRtt field.
func (o *VpnNetworkPing) SetMeanDevRtt(v int32) {
	o.MeanDevRtt = &v
}

// GetMinRtt returns the MinRtt field value if set, zero value otherwise.
func (o *VpnNetworkPing) GetMinRtt() int32 {
	if o == nil || utils.IsNil(o.MinRtt) {
		var ret int32
		return ret
	}
	return *o.MinRtt
}

// GetMinRttOk returns a tuple with the MinRtt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VpnNetworkPing) GetMinRttOk() (*int32, bool) {
	if o == nil || utils.IsNil(o.MinRtt) {
		return nil, false
	}
	return o.MinRtt, true
}

// HasMinRtt returns a boolean if a field has been set.
func (o *VpnNetworkPing) HasMinRtt() bool {
	if o != nil && !utils.IsNil(o.MinRtt) {
		return true
	}

	return false
}

// SetMinRtt gets a reference to the given int32 and assigns it to the MinRtt field.
func (o *VpnNetworkPing) SetMinRtt(v int32) {
	o.MinRtt = &v
}

// GetPktsReceived returns the PktsReceived field value if set, zero value otherwise.
func (o *VpnNetworkPing) GetPktsReceived() int32 {
	if o == nil || utils.IsNil(o.PktsReceived) {
		var ret int32
		return ret
	}
	return *o.PktsReceived
}

// GetPktsReceivedOk returns a tuple with the PktsReceived field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VpnNetworkPing) GetPktsReceivedOk() (*int32, bool) {
	if o == nil || utils.IsNil(o.PktsReceived) {
		return nil, false
	}
	return o.PktsReceived, true
}

// HasPktsReceived returns a boolean if a field has been set.
func (o *VpnNetworkPing) HasPktsReceived() bool {
	if o != nil && !utils.IsNil(o.PktsReceived) {
		return true
	}

	return false
}

// SetPktsReceived gets a reference to the given int32 and assigns it to the PktsReceived field.
func (o *VpnNetworkPing) SetPktsReceived(v int32) {
	o.PktsReceived = &v
}

// GetPktsSent returns the PktsSent field value if set, zero value otherwise.
func (o *VpnNetworkPing) GetPktsSent() int32 {
	if o == nil || utils.IsNil(o.PktsSent) {
		var ret int32
		return ret
	}
	return *o.PktsSent
}

// GetPktsSentOk returns a tuple with the PktsSent field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VpnNetworkPing) GetPktsSentOk() (*int32, bool) {
	if o == nil || utils.IsNil(o.PktsSent) {
		return nil, false
	}
	return o.PktsSent, true
}

// HasPktsSent returns a boolean if a field has been set.
func (o *VpnNetworkPing) HasPktsSent() bool {
	if o != nil && !utils.IsNil(o.PktsSent) {
		return true
	}

	return false
}

// SetPktsSent gets a reference to the given int32 and assigns it to the PktsSent field.
func (o *VpnNetworkPing) SetPktsSent(v int32) {
	o.PktsSent = &v
}

// GetError returns the Error field value if set, zero value otherwise.
func (o *VpnNetworkPing) GetError() string {
	if o == nil || utils.IsNil(o.Error) {
		var ret string
		return ret
	}
	return *o.Error
}

// GetErrorOk returns a tuple with the Error field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VpnNetworkPing) GetErrorOk() (*string, bool) {
	if o == nil || utils.IsNil(o.Error) {
		return nil, false
	}
	return o.Error, true
}

// HasError returns a boolean if a field has been set.
func (o *VpnNetworkPing) HasError() bool {
	if o != nil && !utils.IsNil(o.Error) {
		return true
	}

	return false
}

// SetError gets a reference to the given string and assigns it to the Error field.
func (o *VpnNetworkPing) SetError(v string) {
	o.Error = &v
}

// GetInfoFlags returns the InfoFlags field value if set, zero value otherwise.
func (o *VpnNetworkPing) GetInfoFlags() []string {
	if o == nil || utils.IsNil(o.InfoFlags) {
		var ret []string
		return ret
	}
	return o.InfoFlags
}

// GetInfoFlagsOk returns a tuple with the InfoFlags field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *VpnNetworkPing) GetInfoFlagsOk() ([]string, bool) {
	if o == nil || utils.IsNil(o.InfoFlags) {
		return nil, false
	}
	return o.InfoFlags, true
}

// HasInfoFlags returns a boolean if a field has been set.
func (o *VpnNetworkPing) HasInfoFlags() bool {
	if o != nil && !utils.IsNil(o.InfoFlags) {
		return true
	}

	return false
}

// SetInfoFlags gets a reference to the given []string and assigns it to the InfoFlags field.
func (o *VpnNetworkPing) SetInfoFlags(v []string) {
	o.InfoFlags = v
}

func (o VpnNetworkPing) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o VpnNetworkPing) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !utils.IsNil(o.AvgRtt) {
		toSerialize["avgRtt"] = o.AvgRtt
	}
	if !utils.IsNil(o.MaxRtt) {
		toSerialize["maxRtt"] = o.MaxRtt
	}
	if !utils.IsNil(o.Loss) {
		toSerialize["loss"] = o.Loss
	}
	if !utils.IsNil(o.MeanDevRtt) {
		toSerialize["meanDevRtt"] = o.MeanDevRtt
	}
	if !utils.IsNil(o.MinRtt) {
		toSerialize["minRtt"] = o.MinRtt
	}
	if !utils.IsNil(o.PktsReceived) {
		toSerialize["pktsReceived"] = o.PktsReceived
	}
	if !utils.IsNil(o.PktsSent) {
		toSerialize["pktsSent"] = o.PktsSent
	}
	if !utils.IsNil(o.Error) {
		toSerialize["error"] = o.Error
	}
	if !utils.IsNil(o.InfoFlags) {
		toSerialize["infoFlags"] = o.InfoFlags
	}
	return toSerialize, nil
}

type NullableVpnNetworkPing struct {
	value *VpnNetworkPing
	isSet bool
}

func (v NullableVpnNetworkPing) Get() *VpnNetworkPing {
	return v.value
}

func (v *NullableVpnNetworkPing) Set(val *VpnNetworkPing) {
	v.value = val
	v.isSet = true
}

func (v NullableVpnNetworkPing) IsSet() bool {
	return v.isSet
}

func (v *NullableVpnNetworkPing) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableVpnNetworkPing(val *VpnNetworkPing) *NullableVpnNetworkPing {
	return &NullableVpnNetworkPing{value: val, isSet: true}
}

func (v NullableVpnNetworkPing) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableVpnNetworkPing) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


