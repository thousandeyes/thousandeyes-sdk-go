...
------------- | ------------- | -------------
[**GetAgentsLocalProblems**](LocalProblemsAPI.md#GetAgentsLocalProblems) | **Get** /agents/local-problems | List Cloud agents with local problems



## GetAgentsLocalProblems

> LocalProblemAgentResults GetAgentsLocalProblems().Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Execute()
...
func main() {
	...
	api := (*agents.LocalProblemsAPIService)(&apiClient.Common)

	resp, r, err := api.GetAgentsLocalProblems().Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `LocalProblemsAPI.GetAgentsLocalProblems``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetAgentsLocalProblems`: LocalProblemAgentResults
	json, _ := resp.MarshalJSON()
	fmt.Fprintf(os.Stdout, "Response from `LocalProblemsAPI.GetAgentsLocalProblems`: %v\n", string(json))
}
```
...
Other parameters are passed through a pointer to a ApiGetAgentsLocalProblemsRequest struct via the builder pattern
