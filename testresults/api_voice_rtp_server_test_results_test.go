...
func TestVoiceRTPServerTestResultsAPIService_GetTestRtpServerResults(t *testing.T) {
	...
	var expectedResponse RtpStreamTestResults
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		expectedPath := "/test-results/{testId}/rtp-server"
	...
	api := (*VoiceRTPServerTestResultsAPIService)(&apiClient.Common)

	actualResponse, httpResponse, err := api.GetTestRtpServerResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
	if err != nil {
		t.Fatalf("GetTestRtpServerResults() error = %v", err)
	}
	if httpResponse == nil {
		t.Fatal("GetTestRtpServerResults() returned a nil HTTP response")
	}
	if httpResponse.StatusCode != responseStatus {
		t.Errorf("GetTestRtpServerResults() status = %d, want %d", httpResponse.StatusCode, responseStatus)
	}
	expected := &expectedResponse
	if !reflect.DeepEqual(actualResponse, expected) {
		t.Errorf("GetTestRtpServerResults() response = %#v, want %#v", actualResponse, expected)
	}
}

func TestVoiceRTPServerTestResultsAPIService_GetTestRtpServerResults_Errors(t *testing.T) {
	const testToken = "test-token"
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.GetTestRtpServerResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
		if err == nil {
			t.Fatalf("GetTestRtpServerResults() error = nil, want *GenericAPIError")
		}
		if httpResponse == nil {
			t.Fatal("GetTestRtpServerResults() returned a nil HTTP response")
		}
		...
		if !errors.As(err, &apiError) {
			t.Fatalf("GetTestRtpServerResults() error type = %T, want *GenericAPIError", err)
		}
