...
func TestDNSTraceTestResultsAPIService_GetTestDnsTraceResults(t *testing.T) {
	...
	var expectedResponse DnsTraceTestResults
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		expectedPath := "/test-results/{testId}/dns-trace"
	...
	api := (*DNSTraceTestResultsAPIService)(&apiClient.Common)

	actualResponse, httpResponse, err := api.GetTestDnsTraceResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
	if err != nil {
		t.Fatalf("GetTestDnsTraceResults() error = %v", err)
	}
	if httpResponse == nil {
		t.Fatal("GetTestDnsTraceResults() returned a nil HTTP response")
	}
	if httpResponse.StatusCode != responseStatus {
		t.Errorf("GetTestDnsTraceResults() status = %d, want %d", httpResponse.StatusCode, responseStatus)
	}
	expected := &expectedResponse
	if !reflect.DeepEqual(actualResponse, expected) {
		t.Errorf("GetTestDnsTraceResults() response = %#v, want %#v", actualResponse, expected)
	}
}

func TestDNSTraceTestResultsAPIService_GetTestDnsTraceResults_Errors(t *testing.T) {
	const testToken = "test-token"
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.GetTestDnsTraceResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
		if err == nil {
			t.Fatalf("GetTestDnsTraceResults() error = nil, want *GenericAPIError")
		}
		if httpResponse == nil {
			t.Fatal("GetTestDnsTraceResults() returned a nil HTTP response")
		}
		...
		if !errors.As(err, &apiError) {
			t.Fatalf("GetTestDnsTraceResults() error type = %T, want *GenericAPIError", err)
		}
