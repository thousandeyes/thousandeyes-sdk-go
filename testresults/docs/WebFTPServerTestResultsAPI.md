...
> FtpServerTestResults GetTestFtpServerResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
...
func main() {
	...
	api := (*testresults.WebFTPServerTestResultsAPIService)(&apiClient.Common)

	resp, r, err := api.GetTestFtpServerResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `WebFTPServerTestResultsAPI.GetTestFtpServerResults``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTestFtpServerResults`: FtpServerTestResults
	json, _ := resp.MarshalJSON()
	fmt.Fprintf(os.Stdout, "Response from `WebFTPServerTestResultsAPI.GetTestFtpServerResults`: %v\n", string(json))
}
```
...
Other parameters are passed through a pointer to a ApiGetTestFtpServerResultsRequest struct via the builder pattern
