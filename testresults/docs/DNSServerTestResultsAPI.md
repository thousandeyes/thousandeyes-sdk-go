# DNSServerTestResultsAPI

All URIs are relative to *https://api.thousandeyes.com/v7*

Method | HTTP request | Description
------------- | ------------- | -------------
[**GetTestDnsServerResult**](DNSServerTestResultsAPI.md#GetTestDnsServerResult) | **Get** /test-results/{testId}/dns-server/{serverId} | Get DNS server test results by server
[**GetTestDnsServersResults**](DNSServerTestResultsAPI.md#GetTestDnsServersResults) | **Get** /test-results/{testId}/dns-server | Get DNS server test results



## GetTestDnsServerResult

> DnsServerTestResults GetTestDnsServerResult(testId, serverId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
...
func main() {
	...
	serverId := "281474976710706" // string | DNS server ID
	...
	api := (*testresults.DNSServerTestResultsAPIService)(&apiClient.Common)

	resp, r, err := api.GetTestDnsServerResult(testId, serverId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `DNSServerTestResultsAPI.GetTestDnsServerResult``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTestDnsServerResult`: DnsServerTestResults
	json, _ := resp.MarshalJSON()
	fmt.Fprintf(os.Stdout, "Response from `DNSServerTestResultsAPI.GetTestDnsServerResult`: %v\n", string(json))
}
```
...
------------- | ------------- | ------------- | -------------
**testId** | **string** | Test ID | **serverId** | **string** | DNS server ID | 

### Other Parameters

Other parameters are passed through a pointer to a ApiGetTestDnsServerResultRequest struct via the builder pattern
...
> DnsServerTestResults GetTestDnsServersResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
...
func main() {
	...
	resp, r, err := api.GetTestDnsServersResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `DNSServerTestResultsAPI.GetTestDnsServersResults``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTestDnsServersResults`: DnsServerTestResults
	json, _ := resp.MarshalJSON()
	fmt.Fprintf(os.Stdout, "Response from `DNSServerTestResultsAPI.GetTestDnsServersResults`: %v\n", string(json))
}
```
...
Other parameters are passed through a pointer to a ApiGetTestDnsServersResultsRequest struct via the builder pattern
