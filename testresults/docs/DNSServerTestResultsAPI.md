...
> DnsServerTestResults GetTestDnsServerResult(testIdserverId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
...
func main() {
	...
	resp, r, err := api.GetTestDnsServerResult(testIdserverId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
	if err != nil {
