# DNSTraceTestResultsAPI

All URIs are relative to *https://api.thousandeyes.com/v7*

Method | HTTP request | Description
------------- | ------------- | -------------
[**GetTestDnsTraceResults**](DNSTraceTestResultsAPI.md#GetTestDnsTraceResults) | **Get** /test-results/{testId}/dns-trace | Get DNS trace test results



## GetTestDnsTraceResults

> DnsTraceTestResults GetTestDnsTraceResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
...
func main() {
	...
	api := (*testresults.DNSTraceTestResultsAPIService)(&apiClient.Common)

	resp, r, err := api.GetTestDnsTraceResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `DNSTraceTestResultsAPI.GetTestDnsTraceResults``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTestDnsTraceResults`: DnsTraceTestResults
	json, _ := resp.MarshalJSON()
	fmt.Fprintf(os.Stdout, "Response from `DNSTraceTestResultsAPI.GetTestDnsTraceResults`: %v\n", string(json))
}
```
...
Other parameters are passed through a pointer to a ApiGetTestDnsTraceResultsRequest struct via the builder pattern
