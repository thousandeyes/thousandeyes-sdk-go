# DNSSECTestResultsAPI

All URIs are relative to *https://api.thousandeyes.com/v7*

Method | HTTP request | Description
------------- | ------------- | -------------
[**GetTestDnsSecResults**](DNSSECTestResultsAPI.md#GetTestDnsSecResults) | **Get** /test-results/{testId}/dnssec | Get DNSSEC test results



## GetTestDnsSecResults

> DnssecTestResults GetTestDnsSecResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
...
func main() {
	...
	api := (*testresults.DNSSECTestResultsAPIService)(&apiClient.Common)

	resp, r, err := api.GetTestDnsSecResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `DNSSECTestResultsAPI.GetTestDnsSecResults``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetTestDnsSecResults`: DnssecTestResults
	json, _ := resp.MarshalJSON()
	fmt.Fprintf(os.Stdout, "Response from `DNSSECTestResultsAPI.GetTestDnsSecResults`: %v\n", string(json))
}
```
...
Other parameters are passed through a pointer to a ApiGetTestDnsSecResultsRequest struct via the builder pattern
