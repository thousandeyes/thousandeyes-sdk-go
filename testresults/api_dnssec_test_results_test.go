...
func TestDNSSECTestResultsAPIService_GetTestDnsSecResults(t *testing.T) {
	...
	var expectedResponse DnssecTestResults
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		expectedPath := "/test-results/{testId}/dnssec"
	...
	api := (*DNSSECTestResultsAPIService)(&apiClient.Common)

	actualResponse, httpResponse, err := api.GetTestDnsSecResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
	if err != nil {
		t.Fatalf("GetTestDnsSecResults() error = %v", err)
	}
	if httpResponse == nil {
		t.Fatal("GetTestDnsSecResults() returned a nil HTTP response")
	}
	if httpResponse.StatusCode != responseStatus {
		t.Errorf("GetTestDnsSecResults() status = %d, want %d", httpResponse.StatusCode, responseStatus)
	}
	expected := &expectedResponse
	if !reflect.DeepEqual(actualResponse, expected) {
		t.Errorf("GetTestDnsSecResults() response = %#v, want %#v", actualResponse, expected)
	}
}

func TestDNSSECTestResultsAPIService_GetTestDnsSecResults_Errors(t *testing.T) {
	const testToken = "test-token"
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.GetTestDnsSecResults(testId).Aid(aid).Window(window).StartDate(startDate).EndDate(endDate).Cursor(cursor).Execute()
		if err == nil {
			t.Fatalf("GetTestDnsSecResults() error = nil, want *GenericAPIError")
		}
		if httpResponse == nil {
			t.Fatal("GetTestDnsSecResults() returned a nil HTTP response")
		}
		...
		if !errors.As(err, &apiError) {
			t.Fatalf("GetTestDnsSecResults() error type = %T, want *GenericAPIError", err)
		}
