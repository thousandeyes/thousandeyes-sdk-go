...
func TestHTTPServerEndpointInstantScheduledTestsAPIService_CreateHttpServerScheduledInstantTest(t *testing.T) {
	...
	var requestBody EndpointHttpServerInstantTest
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		expectedPath := "/endpoint/tests/scheduled-tests/http-server/instant"
		...
		var receivedBody EndpointHttpServerInstantTest
	...
	api := (*HTTPServerEndpointInstantScheduledTestsAPIService)(&apiClient.Common)

	actualResponse, httpResponse, err := api.CreateHttpServerScheduledInstantTest().EndpointHttpServerInstantTest(requestBody).Aid(aid).Execute()
	if err != nil {
		t.Fatalf("CreateHttpServerScheduledInstantTest() error = %v", err)
	}
	if httpResponse == nil {
		t.Fatal("CreateHttpServerScheduledInstantTest() returned a nil HTTP response")
	}
	if httpResponse.StatusCode != responseStatus {
		t.Errorf("CreateHttpServerScheduledInstantTest() status = %d, want %d", httpResponse.StatusCode, responseStatus)
	}
	expected := &expectedResponse
	if !reflect.DeepEqual(actualResponse, expected) {
		t.Errorf("CreateHttpServerScheduledInstantTest() response = %#v, want %#v", actualResponse, expected)
	}
}

func TestHTTPServerEndpointInstantScheduledTestsAPIService_CreateHttpServerScheduledInstantTest_Errors(t *testing.T) {
	const testToken = "test-token"
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.CreateHttpServerScheduledInstantTest().EndpointHttpServerInstantTest(requestBody).Aid(aid).Execute()
		if err == nil {
			t.Fatalf("CreateHttpServerScheduledInstantTest() error = nil, want *GenericAPIError")
		}
		if httpResponse == nil {
			t.Fatal("CreateHttpServerScheduledInstantTest() returned a nil HTTP response")
		}
		...
		if !errors.As(err, &apiError) {
			t.Fatalf("CreateHttpServerScheduledInstantTest() error type = %T, want *GenericAPIError", err)
		}
