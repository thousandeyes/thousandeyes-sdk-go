...
func TestPermissionsAPIService_GetPermissions(t *testing.T) {
	...
	var expectedResponse Permissions
	...
	api := (*PermissionsAPIService)(&apiClient.Common)

	actualResponse, httpResponse, err := api.GetPermissions().Aid(aid).Execute()
	if err != nil {
		t.Fatalf("GetPermissions() error = %v", err)
	}
	if httpResponse == nil {
		t.Fatal("GetPermissions() returned a nil HTTP response")
	}
	if httpResponse.StatusCode != responseStatus {
		t.Errorf("GetPermissions() status = %d, want %d", httpResponse.StatusCode, responseStatus)
	}
	expected := &expectedResponse
	if !reflect.DeepEqual(actualResponse, expected) {
		t.Errorf("GetPermissions() response = %#v, want %#v", actualResponse, expected)
	}
}

func TestPermissionsAPIService_GetPermissions_Errors(t *testing.T) {
	const testToken = "test-token"
	...
	runErrorCase := func(
		...
		_, httpResponse, err := api.GetPermissions().Aid(aid).Execute()
		if err == nil {
			t.Fatalf("GetPermissions() error = nil, want *GenericAPIError")
		}
		if httpResponse == nil {
			t.Fatal("GetPermissions() returned a nil HTTP response")
		}
		...
		if !errors.As(err, &apiError) {
			t.Fatalf("GetPermissions() error type = %T, want *GenericAPIError", err)
		}
